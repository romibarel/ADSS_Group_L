...
public class FixedDaysSupplier extends Supplier {
    ...
    public boolean setProductAmount(int productID, int amount, Order o){
        if(LocalDateTime.now().isBefore(o.getETA().minusDays(1)))//o.getETA().isAfter(LocalDateTime.now().plusDays(1))) // Check if order is at least one day before arrival
    ...
    public boolean setOrderETA(Order o, LocalDateTime ETA){
        if(LocalDateTime.now().isBefore(o.getETA().minusDays(1)))
            return o.setETA(ETA);
